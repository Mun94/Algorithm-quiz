...
function solution(skill = skill.toUpperCase(), skill_trees) {
  let result = 0;
  const order = skill_trees.map((tree) => {
    return [...new Set(skill)].map((sk) => {
      return tree.indexOf(sk);
    });
  });
